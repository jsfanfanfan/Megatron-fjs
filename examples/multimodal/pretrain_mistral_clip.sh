...
if [[ -z $WORKSPACE ]]; then # if 判断变量是否存在
...
"
# 在NVIDIA的Transformer Engine（NVTE）中，环境变量 
# NVTE_APPLY_QK_LAYER_SCALING 是用于控制 QK Layer Scaling 功能的开关
export NVTE_APPLY_QK_LAYER_SCALING=0
# 环境变量 NVTE_ALLOW_NONDETERMINISTIC_ALGO 用于
# 控制是否允许在执行计算时使用 非确定性（Nondeterministic） 的算法
