...
def initialize_model_parallel(
    ...
    参数：
        ...
        pipeline_model_parallel_size (int, default = 1):
            用于在多个张量并行 GPU 组之间划分 
            Transformer 层的 GPU 组数量。
            举例来说，如果 tensor_model_parallel_size
            为 4 且 pipeline_model_parallel_size 为 2，
            则模型将被划分为 2 个由 4 个 GPU 组成的组。

        virtual_pipeline_model_parallel_size (int, optional):
            每个流水线组将包含的阶段数，并在必要时进行
            交错（interleaving）。如果为 None，则不进行交错。
            例如，如果 tensor_model_parallel_size 为 1，
            pipeline_model_parallel_size 为 4，
            virtual_pipeline_model_parallel_size 为 2，
            并且模型中有 16 层 Transformer 层，那么模型将被划分为 8 个阶段，
            每个阶段包含 2 层，每个 GPU 将分配到 2 个阶段，
            具体分配如下（层编号从 1 开始）
        ...
        use_sharp (bool, default = False):
            设置在数据并行进程组的集合通信中使用 SHARP
            （Scalable Hierarchical Aggregation and Reduction Protocol）。
            当设置为 True 时，会在每个数据并行进程组中运行屏障（barrier），
            以指定 SHARP 的应用目标组。

        context_parallel_size (int, default = 1):
            用于划分网络输入序列长度的张量并行 GPU 组的数量。
            注意力模块的计算需要完整序列长度的 token，
            因此，在 context parallel group 中的 GPU 需
            要相互通信，以交换其他序列块的信息。
            每个 GPU 及其在其他张量并行组中的对应 GPU 共同
            组成一个 context parallel group。

            例如，假设我们有 8 个 GPU，
            如果张量模型并行大小为 4，上下文并行大小为 2，
            则网络输入将被划分为两个序列块，
            这些序列块由两个不同的 4 GPU 组进行处理。
            一个块由 GPU0-3 处理，另一个块由 GPU4-7 处理。
            构建四个组进行上下文并行通信：[GPU0, GPU4]， [GPU1, GPU5]，
            [GPU2, GPU6]，和 [GPU3, GPU7]。

            Context parallelism 将序列长度进行划分，因此对权重没有影响，
            这意味着权重会在 context parallel group 中的 GPU 之间进行复制。
            因此，在反向传播过程中需要进行权重梯度的
            全归约（all-reduce）。为了简化操作，
            我们将上下文并行的 GPU 与数据并行组的 GPU 
            一起用于权重梯度的全归约。

        expert_model_parallel_size (int, default = 1):
            每个专家并行组中专家的并行 GPU 数量


        nccl_communicator_config_path (str, default = None):
            NCCL 通信器配置的 YAML 文件路径。
            可以为每个通信器设置 min_ctas、max_ctas 
            和 cga_cluster_size。

        distributed_timeout_minutes (int, default = 30):
            分布式进程组操作的超时时间，
            以分钟为单位。有关注意事项，
            请参阅 PyTorch 文档：https://pytorch.org/docs/stable/distributed.html    

        order (str, default=tp-dp-pp):
            并行化的排名初始化顺序。
            目前支持 tp-dp-pp 和 tp-pp-dp 顺序

        encoder_tensor_model_parallel_size (int, default = 0):
            在编码器中，将单个张量拆分到的 GPU 数量。如果为 0，
            则使用默认值，即解码器的张量模型并行大小。

        encoder_pipeline_model_parallel_size (int, default = 0):
            分配给编码器的张量并行 GPU 组的数量。
            举例来说，如果 pipeline_model_parallel_size 为 4，
            encoder_pipeline_model_parallel_size 为 2，
            那么编码器将使用前两个流水线阶段来处理其层，并且总的流水线数量为 6
    ...
    # Rank Generator:支持 tp-pp-dp 有必要吗？
    ...
    global _PIPELINE_MODELPARALLEL_GROUP
    ...
    for ranks in generator_wrapper('pp'):
        ...
        if rank in ranks:
            ...
            elif isinstance(_PIPELINE_GLOBAL_RANKS[0], list): # 为什么流水线并行有这个情况？
...
def get_amax_reduction_group(with_context_parallel=False, tp_only_amax_red=False):
    """获取调用 rank 属于哪个 FP8 amax reduction group."""
    if with_context_parallel:
...
def get_data_modulo_expert_parallel_group(with_context_parallel=False):
    ...
    if with_context_parallel: # data + context
        assert (
