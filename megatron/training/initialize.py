...
def initialize_megatron(
    ...
    """
    设置全局变量、初始化分布式环境，并设置自动恢复和随机种子。

    allow_no_cuda 不应被设置，除非使用 Megatron 进行仅 CPU 的数据处理。
    通常情况下，不应设置此参数，除非你非常清楚自己在做什么。
    返回一个函数，用于最终初始化分布式环境
    （可选，仅在 args.lazy_mpu_init == True 时执行）
    """
    ...
    if args.use_checkpoint_args or args_defaults.get("use_checkpoint_args", False): # False 不改成 True？
...
def _initialize_tp_communicators():
    ...
    if args.tp_comm_overlap_cfg is not None:
        with open(args.tp_comm_overlap_cfg, "r") as stream:
            ub_cfgs = yaml.safe_load(stream) # ub = user buffer
    else:
        ub_cfgs = {}
...
def _initialize_distributed(get_embedding_ranks, get_position_embedding_ranks):
    ...
    if torch.distributed.is_initialized():
        ...
        if args.rank == 0: # 这里没有 args.rank 吧
    ...
    if device_count > 0:
        if mpu.model_parallel_is_initialized():
            print("model_parallel is already initialized")
        else:
